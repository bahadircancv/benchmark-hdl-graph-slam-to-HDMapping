...
int main(int argc, char **argv)
{
    ...
    for (const rosbag::MessageInstance& m : view) {
        if (m.getTopic() == "/aligned_points") {
            ROS_INFO("Received message on topic: /aligned_points");
        ...
        if (m.getTopic() == "/odom") {
            ROS_INFO("Received message on topic: /odom");
    ...
    for (int i = 0; i < chunks_pc.size(); i++)
    {
        ...
        fs::path path(outwd);
        std::string filename = ("scan_hdl_graph_slam_" + std::to_string(i) + ".laz");
        path /= filename;
        std::cout << "saving to: " << path << " number of points: " << chunks_pc[i].size() << std::endl;
        saveLaz(path.string(), chunks_pc[i]);
        file_names.push_back(filename);

        std::string trajectory_filename = ("trajectory_hdl_graph_slam_" + std::to_string(i) + ".csv");
    ...
    fs::path path(outwd);
    path /= "hdl_graph_slam_initial_poses.reg";
