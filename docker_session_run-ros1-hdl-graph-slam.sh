...
IMAGE_NAME='hdl-graph-slam_noetic'
TMUX_SESSION='ros1_session'

DATASET_CONTAINER_PATH='/ros_ws/dataset/recorded-hdl-graph-slam.bag'
BAG_OUTPUT_CONTAINER='/ros_ws/recordings'

RECORDED_BAG_NAME="recorded-hdl-graph-slam.bag"
...
echo "=== HDL-GRAPH-SLAM rosbag pipeline ==="
...
roslaunch hdl_graph_slam hdl_graph_slam.launch use_sim_time:=true
'\'' C-m
...
source /ros_ws/devel/setup.bash
echo "[record] start"
rosbag record /aligned_points /odom -O '"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME"'
echo "[record] exit"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    source /opt/ros/noetic/setup.bash
    source /ros_ws/devel/setup.bash
    rosrun hdl-graph-slam-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-hdl-graph-slam\"
  "
